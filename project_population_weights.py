...
def load_data(distribution_center_file, population_file):
    ...
    county_data = load_csv(
        population_file, ['County', 'Latitude', 'Longitude'])
    distribution_data = load_csv(distribution_center_file, [
...
def find_closest_center(distribution_data, county_data):
    ...
    Ranks the closest vaccine distribution center to a zip code by distance
        ...
        PARAM:
            distribution_data (dataframe) - maps dist sites to their coords
            county_data (dataframe) - maps counties to their coords

        RETURNS:
            np array with index i as zipcode i. array[i] is an array of tuples (center j, dist to center j from zipcode i)
    '''
    county_to_closest_center = np.empty(len(county_data))

    for county_i, county_row in county_data.iterrows():
        # get county data
        county, num, lat, lon = county_row['County [2]'], county_row[
            'Number 65+ Population'], county_row['Latitude'], county_row['Longitude']
        ranked = []
        ...
        # save this zipcode's closest centers
        county_to_closest_center[county_i] = ranked
...
def build_bqm(potential_new_dist_nodes, num_poi, pois, num_dist, dist_sites, num_new_dist):
    ...
    # Build BQM using adjVectors to find best new distribution location s.t. min
    # distance to POIs and max distance to existing distribution locations
    ...
    # Constraint 2: Max distance to existing distribution sites
    if num_dist > 0:
        for i in range(len(potential_new_dist_nodes)):
            # Compute average distance to POIs from this node
            avg_dist = 0
            cand_loc = potential_new_dist_nodes[i]
            for loc in dist_sites:
                dist = (-1*cand_loc[0]**2 + 2*cand_loc[0]*loc[0] - loc[0]**2
                        - cand_loc[1]**2 + 2*cand_loc[1]*loc[1] - loc[1]**2)
                avg_dist += dist / num_dist
            bqm.linear[i] += avg_dist * gamma2

    # Constraint 3: Choose exactly num_new_dist new distribution locations
...
def printout_solution_to_cmdline(pois, num_poi, dist_sites, num_dist, new_dist_nodes, num_new_dist):
    """ 
    ...
    if num_dist > 0:
        old_cs_avg_dist = [sum(abs(a - b) for a, b in zip(new, loc)
                               for loc in dist_sites) / num_dist for new in new_dist_nodes]
        print("Average distance to old distribution sites:\t", old_cs_avg_dist)
    ...
    if num_new_dist > 1:
        ...
        print("Distance between new distribution sites:\t\t\t", new_cs_dist)


def run_bqm_and_collect_solutions(bqm, sampler, potential_new_dist_nodes, **kwargs):
    """ Solve the bqm with the provided sampler to find new distribution locations. """
...
def iterate_bqm_and_graph(iters, run_num):
    """
    Solve the BQM and generate graphs of the new distribution sites in the
    results folder. 

        PARAMS:
            iters (int) - number of times to repeat calculation
            run_num (int) - folder name
    """

    step = 77 // iters
    d, c = load_data('data/Distribution_center_locations_TX.csv',
    ...
    for i in range(0, 77, step):
        ...
        poi_avg_dist = printout_solution_to_cmdline(counties, len(counties), [], 0, new_dist_nodes,
                                                    len(new_dist_nodes))
        ...
        print("distribution_nodes: {}".format(new_dist_nodes))
        print("poi_distance: {}".format(poi_avg_dist))

        try:
            np.savetxt("results/sites_{}.txt".format(i), new_dist_nodes)
